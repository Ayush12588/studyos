...
const App={
    ...
    renderDashboard(){
        ...
        ${savedHTML}`;
            ...
            el.innerHTML=`<div style="padding:40px;text-align:center">
                ...
                <h2 style="font-size:1.3rem;margin-bottom:8px">Welcome to StudyOS!</h2>
    ...
    renderSubjects(){
        ...
        if(subs.length===0){h+=`<div class="empty-state"><span class="empty-state-icon">📚</span><div class="empty-state-title">No subjects yet</div><div class="empty-state-desc">Load your entire CBSE Class ${this.state.profile.selectedClass||10} syllabus in one tap, or add subjects manually.</div><div style="display:flex;gap:var(--sp-2);justify-content:center;flex-wrap:wrap"><button class="btn btn-primary" onclick="App._welcomeClass=App.state.profile.selectedClass||10;App._welcomeStream=App.state.profile.selectedStream||null;App.loadCBSEForClass()">Load CBSE Class ${this.state.profile.selectedClass||10} Syllabus</button><button class="btn btn-secondary" onclick="App.openModal('modal-subject')">+ Add Manually</button></div><div class="empty-state-hint">💡 CBSE loads all 5 subjects with chapters pre-filled</div></div>`}
        else{flt.forEach(s=>{
...
<div class="form-group"><label class="form-label">Notes</label><input type="text" id="log-notes" class="form-input" placeholder="Quick note..."></div>`;
    ...
    renderLog(){
        ...
        if(ss.length===0){h+=`<div class="empty-state"><span class="empty-state-icon">✏️</span><div class="empty-state-title">No sessions logged yet</div><div class="empty-state-desc">Every study session you log builds your streak, earns XP, and helps the AI Coach give you better advice.</div><button class="btn btn-primary" onclick="App.openQuickLog()">Log Your First Session</button><div class="empty-state-hint">⏱ Even 15 minutes counts!</div></div>`}
    ...
    renderTasks(){
        ...
        if(todayTasks.length===0){h+=`<div class="empty-state"><span class="empty-state-icon">☑️</span><div class="empty-state-title">No tasks for today</div><div class="empty-state-desc">Tasks help you stay focused on what matters. Add your own or let AI plan your day automatically.</div><button class="btn btn-primary" onclick="App.autoGenerateTasks()">Auto-Plan My Day</button><div class="empty-state-hint">💡 Tip: Auto-Plan pulls from your overdue chapters and revisions</div></div>`}
    ...
    renderDoubts(){
        ...
        if(doubts.length===0){
            h+=`<div class="empty-state"><span class="empty-state-icon">🤔</span><div class="empty-state-title">No doubts logged — good sign!</div><div class="empty-state-desc">When you're confused about something, log it here. Your AI Coach can help explain it and you'll never forget to revisit a tricky concept.</div><button class="btn btn-primary" onclick="App.openDoubtModal()">+ Log a Doubt</button><div class="empty-state-hint">💡 Clearing doubts before boards is critical — don't skip this</div></div>`;
        }
        el.innerHTML=h;
    },
    openDoubtModal(){
    ...
    renderResources(){
        ...
        if(resources.length===0){
            h+=`<div class="empty-state"><span class="empty-state-icon">🔗</span><div class="empty-state-title">No resources saved yet</div><div class="empty-state-desc">Save YouTube videos, PDFs, and websites here so you never lose a good study resource mid-session.</div><button class="btn btn-primary" onclick="App.openResourceModal()">+ Save First Resource</button><div class="empty-state-hint">💡 Great resources: NCERT PDFs, PW videos, Khan Academy</div></div>`;
        }else{
    ...
    renderNotes(){
        ...
        if(notes.length===0){
            h+=`<div class="empty-state"><span class="empty-state-icon">📓</span><div class="empty-state-title">No notes yet</div><div class="empty-state-desc">Save formulas, key concepts, and revision notes here. You can attach images and PDFs too.</div><button class="btn btn-primary" onclick="App.openNoteModal()">+ Write First Note</button><div class="empty-state-hint">💡 Try saving the quadratic formula or Newton's laws first</div></div>`;
        }else{
            // Search notes
...
<div class="coach-drawer" id="coach-drawer">
    ...
    renderSettings(){
        ...
        <div class="card"><div class="card-header"><span class="card-title">About StudyOS</span></div><p style="font-size:.82rem;color:var(--text-secondary);line-height:1.8">StudyOS is your personal study OS. Track subjects, chapters, revisions, exams, doubts, and more — all in one place.</p><p style="font-size:.75rem;color:var(--text-muted);margin-top:8px">All data is stored locally in your browser. Export regularly to avoid data loss!</p></div></div></div>`;
    },
    ...
    exportData(){
        ...
        a.href=u;a.download=`studyos-${this.today()}.json`;a.click();URL.revokeObjectURL(u);this.toast('Data exported! 📤','success');
    },
    importData(e){
